...
function renderProfileFromTalk(talk = DETAILS_TALK){
  ...
  const label = talk.desc || "自己紹介";
  const message_profile = talk.message_profile || "";

  host.innerHTML = `
    <div class="profile-card">
      <div class="p-left">
        <img src="${photo}" alt="${name ? name + '�i�X���j' : '�X��'}" loading="lazy" decoding="async">
        ${name ? `<div class="badge">${name}</div>` : ""}
      </div>
      <div class="p-right">
        <p class="desc">
          <span class="profile-label">${label}</span>
          <span class="profile-text">${message_profile}</span>
        </p>
      </div>
    </div>`;
}
...
function renderTOC(){
  ...
  const sections = all
    .map(sec => {
      ...
      const headingEl = sec.querySelector("h2, h1");
    ...
    .filter(({ id, heading }) => {
      ...
      if (id === "profile" || normalized === "profile" || heading === "自己紹介") {
        return false;
      }
      seen.add(id);
      return true;
...
function renderRanking(){
  ...
  host.innerHTML = RANKING.map((r,i)=>{
    ...
    const rank = i + 1;
    const crown = rank <= 5 ? '<span class="crown" aria-hidden="true">👑</span>' : '';
    const inner = `
...
function renderCompare(){
  ...
  setupCompareScrollbar(host);
}

function setupCompareScrollbar(container){
  const scrollbar = container.querySelector(".table-scrollbar");
  const thumb = scrollbar?.querySelector(".thumb") || scrollbar?.querySelector("span");
  ...
  const getTrackWidth = () => {
    ...
    return scrollbar.clientWidth - paddingLeft - paddingRight;
  };
  ...
  const update = () => {
    ...
    const thumbWidth = Math.max(trackWidth * container.clientWidth / container.scrollWidth, 36);
    const maxThumbOffset = Math.max(trackWidth - thumbWidth, 0);
    const offset = maxScroll ? (container.scrollLeft / maxScroll) * maxThumbOffset : 0;
  ...
  if (typeof ResizeObserver === "function"){
    ...
    const inner = container.querySelector(".table-host-inner");
...
function initDetailCarousel(list = PRODUCTS){
  ...
  const specLines = (p) => {
    const rows = [];
    rows.push(`価額:${fmtPrice(p.price)}`);
    
    if (p.specs?.コース数) rows.push(`コース数:${p.specs?.コース数}`);
    if (p.specs?.掲載点数) rows.push(`掲載点数:${p.specs?.掲載点数}`);
    if (p.specs?.取扱ジャンル) rows.push(`取扱ジャンル:${p.specs?.取扱ジャンル}`);
    if (p.specs?.おすすめのシーン) rows.push(`おすすめのシーン:${p.specs?.おすすめのシーン}`);
    if (p.specs?.有効期限) rows.push(`有効期限:${p.specs?.有効期限}`);
    return rows.map(line => {
      const parts = line.split(/[:]/);
      if (parts.length < 2) return `<li>${line}</li>`;
      const [label, ...rest] = parts;
      const value = rest.join(":").trim();
      return `<li><span class="spec-key">${label}</span><span class="spec-value">${value}</span></li>`;
    }).join("");
  };
  ...
  track.innerHTML = list.map(p => `
    <figure class="dc-stage">
      ...
      <span class="dc-intro">${p.intro || ""}</span>
