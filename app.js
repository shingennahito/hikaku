...
/* ============================================================================
   ...
   ============================================================================ */
// --- THAY THẾ TOÀN BỘ HÀM renderCompare() CŨ ---
...
// Phân trang số có rút gọn bằng "…"
function makePagerModel(total, current){
  const pages = [];
  const cur = current + 1; // current là chỉ số 0-based, hiển thị 1-based
  const windowSize = 2;    // số trang kề hai bên current
  const keep = new Set([1, 2, total-1, total, cur-2, cur-1, cur, cur+1, cur+2]
...
function renderPager(){
  ...
  el.dots.innerHTML = items.map(item => {
    ...
    // item là số trang (1-based)
    const isCur = (item === cur + 1) ? 'is-current' : '';
    return `<button class="pg-num ${isCur}" data-page="${item}">${item}</button>`;
  }).join('');

  // Gán sự kiện
  el.dots.querySelectorAll('.pg-num').forEach(btn => {
    btn.addEventListener('click', () => {
      const page = parseInt(btn.dataset.page, 10);
      go(page - 1);
    });
  });
  el.dots.querySelector('[data-act="prev"]')?.addEventListener('click', () => go(cur - 1));
  el.dots.querySelector('[data-act="next"]')?.addEventListener('click', () => go(cur + 1));
}
